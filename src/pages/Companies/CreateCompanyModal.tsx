...
export default function CreateCompanyModal({
    ...
    useEffect(() => {
        if (!open) return;
        setName(''); setLogoRel(''); setLogoPreview(''); setMsg(null);
    ...
    return (
        <Modal isOpen={open} toggle={onClose} centered modalClassName="modal-compact">
            <ModalHeader toggle={onClose}>Create Company</ModalHeader>
            <ModalBody>
                <style>{`
        ...
        `}</style>
                ...
                <Form onSubmit={onSubmit} className="d-flex flex-column gap-3">
                    ...
                    <div className="d-grid gap-2 mt-2">
                        <Button type="submit" color="primary" disabled={saving || uploading || !name.trim()}>{saving ? 'Creating...' : 'Create'}</Button>
